...
from backend.core.config.constants import API_PREFIX, APP_NAME, APP_VERSION, APP_DEBUG
from backend.core.events import startup_handler, shutdown_handler


def get_api() -> FastAPI:
    fast_api = FastAPI(title=APP_NAME, version=APP_VERSION, debug=APP_DEBUG)
