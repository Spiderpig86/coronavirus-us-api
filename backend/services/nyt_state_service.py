"""New York Times Information Retriever for States
...
class NytStatesService(object):
    def __init__(self):
        self.ENDPOINT = DATA_ENDPOINTS.get(self.__class__.__name__)

    @cached(cache=TTLCache(maxsize=1024, ttl=3600)) # TODO: Convert to constant
    async def get_data(self):
        ...
        csv_data = ""
        logger.info("Fetching NYT state data...")
    ...
    def group_location(self, csv_data: List):
        ...
        for timestamp in csv_data:
            location_id = (timestamp["state"], timestamp["fips"])
            ...
            # Collect stats for same location
    ...
    def _location_id(
        ...
        """Generates string ID given tuple containing state and FIPS code.
        
        Arguments:
            tuple_id {tuple} -- tuple containing state and FIPS code.
        
        Returns:
            str -- string ID representation.
        """
        return f"{tuple_id[0]}@{tuple_id[1]}"
